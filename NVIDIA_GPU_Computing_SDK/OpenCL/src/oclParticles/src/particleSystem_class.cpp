...
void ParticleSystem::_initialize(int numParticles){
    ...
    if (!m_bQATest)
    {
        ...
        float *data = (float*)malloc(nParticles*4*sizeof(float));//(float *)glMapBuffer(GL_ARRAY_BUFFER, GL_WRITE_ONLY);
            float *ptr = data;
        ...
        glBufferData(GL_ARRAY_BUFFER, nParticles*4*sizeof(float), data, GL_DYNAMIC_DRAW);//glUnmapBuffer(GL_ARRAY_BUFFER);
        */
    }
...
void ParticleSystem::update(float deltaTime){
    ...
    if (!m_bQATest)
    {
        ...
        //pos = (memHandle_t)glMapBuffer(GL_ARRAY_BUFFER, GL_READ_WRITE);
