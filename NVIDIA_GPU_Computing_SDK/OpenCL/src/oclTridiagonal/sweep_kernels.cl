...
__kernel void transpose(__global float *odata, __global float *idata, int width, int height, __local float *block)
{
	...
	int i0 = MUL_24(blockIdxx, BLOCK_DIM) + threadIdxx;
	int j0 = MUL_24(blockIdxy, BLOCK_DIM) + threadIdxy;
	
	if (i0 >= width || j0 >= height) return;

	int i1 = MUL_24(blockIdxy, BLOCK_DIM) + threadIdxx;
    int j1 = MUL_24(blockIdxx, BLOCK_DIM) + threadIdxy;
    
	if (i1 >= height || j1 >= width) return;

	int idx_a = i0 + MUL_24(j0, width);
    int idx_b = i1 + MUL_24(j1, height);
