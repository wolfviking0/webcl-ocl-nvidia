...
int main(int argc, char** argv)
{
    ...
    #ifdef __EMSCRIPTEN__
        uiOutput = (unsigned int *)malloc(uiImageWidth * uiImageHeight * sizeof(unsigned int));
    #endif
...
void InitGlut(int* argc, char **argv)
{
    ...
    printf("Increase Filter Radius [+]\n");
    printf("Decrease Filter Radius [-]\n");
    printf("Toggle Processing between GPU and CPU [p]\n");
    printf("Toggle OpenGL interop [g]\n");
    ...
    if (bFilter)
    {
        ...
        if (iProcFlag == 0)
        {
            ...
            else 
            {
                    ...
                    void* uiOutput = glMapBuffer(GL_PIXEL_UNPACK_BUFFER_ARB, GL_WRITE_ONLY_ARB);
        ...
        else 
        {
            // Bind and map PBO for output
                ...
                unsigned int* uiOutput = (unsigned int*)glMapBuffer(GL_PIXEL_UNPACK_BUFFER_ARB, GL_WRITE_ONLY_ARB);
