...
static void myFghTeapot( GLint grid, GLdouble scale, GLenum type )
{
...
int main(int argc, char** argv) 
{
    ...
    if(!bQATest) 
    {
        ...
        source_ptr = (unsigned int*) malloc(image_width * image_height * sizeof(int));
        dest_ptr = (unsigned int*) malloc(image_width * image_height * sizeof(int));
...
bool InitGL(int* argc, char **argv )
{
    ...
    // create GLUT menu
    #ifdef __EMSCRIPTEN__
    printf("Toggle Post-processing (Blur filter) ON/OFF <spacebar>\n");
    printf("Toggle Processor between GPU and CPU [p]\n");
    printf("Toggle GL animation (rotation) ON/OFF [a]\n");
    printf("Increment blur radius [+ or =]\n");
    printf("Decrement blur radius [- or _]\n");
    printf("Quit <esc>\n");
    #else
    ...
    glMaterialfv(GL_FRONT_AND_BACK, GL_SHININESS, other);
    #endif
...
void processImage()
{
    ...
    glReadPixels(0, 0, image_width, image_height, GL_RGBA, GL_UNSIGNED_BYTE, source_ptr); 
    ...
    if (bPostprocess)
    {
        ...
        glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, 
                        ...
                        GL_RGBA, GL_UNSIGNED_BYTE, dest_ptr);
    ...
    else 
    {
        ...
        glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, 
                        image_width, image_height, 
                        GL_RGBA, GL_UNSIGNED_BYTE, source_ptr);
